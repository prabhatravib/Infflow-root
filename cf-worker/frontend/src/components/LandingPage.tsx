...
import { Search, Mic, Image, Moon, Sun, PlayCircle } from 'lucide-react';
      ...
      <div className="w-full max-w-4xl flex flex-col items-center">
        ...
        {onStartAutoDemo && (
          <button
            ...
            disabled={autoDemoActive}
            className={`mt-8 px-6 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-2xl font-medium shadow-xl transition-all duration-200 flex items-center gap-2 ${
              autoDemoActive ? 'opacity-70 cursor-not-allowed' : 'hover:from-purple-700 hover:to-blue-700'
            }`}
          >
            <PlayCircle className="w-5 h-5" />
            <span>{autoDemoActive ? 'Demo Playing' : 'Start AutoDemo'}</span>
          </button>
        )}
      </div>
    </motion.div>
